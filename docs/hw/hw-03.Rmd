...
4. Suppose we want to test if the mean winning speed is the same for fast track and good track conditions when the number of starters is held constant and  `year = 0`. What is the p-value assoicated with the test? 

5. Interpret the coefficient of `conditionslow`. Assume `year = 0`. 
...
6. Use the information from the table to calculate the 95% confidence interval for the amount by which the slope of year for slow track conditions exceeds the slope of year for fast track conditions. *Hint: You can caluclate t\* using the `qt()` function.*
