...
- Go to the STA210-Sp19 organization on GitHub ([https://github.com/STA210-Sp19](https://github.com/STA210-Sp19)). Click on the repo with the prefix **hw-05**. It contains the starter documents you need to complete the lab.
...
7. Data in `NBA1718team.csv` (“NBA Enhanced Box Scores and Standings” 2018) looks at factors that are associated with a team’s winning percentage. You will use the data to build a model that could be used to predict a team's win percentage. The following variables are in the data set. 
...
Click [here](https://www2.stat.duke.edu/courses/Spring19/sta210.001/hw/data/NBA1718team.csv) to download the data. Upload the data into the `data` folder in your RStudio Cloud project. 

- Use `win_pct` to create a new variable that is the log-odds of winning. This will be the response variable for the analysis. 
- Use the `step` function to conduct backward selection to choose a final model that can be used to predict win percentage. You want to define the model in such a way that the intercept has a reasonable and meaningful interpretation. Be sure to output the coefficients of the final model along with its AIC value. 
- Based on this model, discuss which factors are important for predicting an NBA team's win percentage.
- What is the predicted win percentage (and corresponding interval) for a team with average box statistics (e.g. avg free throw percentage, etc.)? 
...
The questions from this assignment are modified from exercises in Chapter 6 of  [https://github.com/proback/BYSH](https://github.com/proback/BYSH).
