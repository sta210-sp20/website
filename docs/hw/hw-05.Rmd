...
7. Data in `NBA1718team.csv` (“NBA Enhanced Box Scores and Standings” 2018) looks at factors that are associated with a team’s winning percentage. You will use the data to build a model that could be used to predict a team's win percentage. You will use the following variables in the dataset: 
...
-  Given the structure of the data, you will approach the logistic regression in a slightly different way than what we've seen thus far.  Use `win_pct` to create a new variable that is the log odds of winning. You will use the `lm` function to build a linear model with this new variable as the repsonse.
