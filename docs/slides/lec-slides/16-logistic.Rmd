...
p_c <- flying %>% 
  filter(age %in% c("18-29", "30-44")) %>%
  summarise(p_c = round(sum(as.numeric(rude))/n(),3)) %>% pull()
```
...
odds <- flying %>% 
  filter(age %in% c("18-29", "30-44")) %>%
  group_by(age) %>%
  summarise(n = n(), p_hat = round(sum(as.numeric(rude))/n,3),
...
- **The odds of $y$ for group $y$ are expected to be $\exp\{\beta_1\}$ times the odds of $y$ for the baseline group.**
