...
- Labs start tomorrow. Make sure you...
  - can access the STA210 workspace on RStudio Cloud
  - are a member of `sta210-fa19` organization on GitHub before
...
- A study was conducted to test whether the show as effective in helping children improve their reading and math skills. The 240 children who participated in the student were split into two groups: 
      + **Group 1**: Those who were encouraged to watch the show
...
- We will focus on the change in reading scores
<br><br>
...
sesame_street <- sesame_street %>% 
  mutate(treatment=as.factor(viewenc), change = postlet - prelet) %>%
...
ggplot(data = sesame_street, mapping = aes(y = change, x = treatment)) +
  ...
  labs(title = "Change in Reading Scores", 
       subtitle = "Group 1: Encouraged to watch, Group 2: Not encouraged to watch") +
  theme_minimal()
```
</small>
...
sesame_street %>%
  group_by(treatment) %>%
  summarise(n = n(), mean = mean(change), s = sd(change))
```
<br>
...
.instructions[
Do children who regularly watch *Sesame Street* have significantly greater improvement in reading scores on average than those who don't?
]
...
- <font class = "vocab">Alternative hypothesis, $H_a$</font>: This is typically what you want to show, i.e. the "there is something going on" hypothesis 
    - In regression, the alternative hypothesis is often "there is some relationship between the two variables"
...
- **Question: ** Do children who regularly watch *Sesame Street* have significantly greater improvement in reading scores on average than those who don't?
--

- **Baseline: ** There is no difference between the two groups
--

- **Claim: **The average change in reading scores for children who watch *Sesame Street* is greater than the average change of children who don't. We will use encouragement as a proxy for watching the show.
--
...
.question[
What is your conclusion about the effectiveness of *Sesame Street* in helping students improve their reading skis?
]
...
- One approach is to simulate the distributions as we did earlier, but we will rely on theoretical results. 

--

- The <font class = "vocab">Central Limit Theorem </font> is a theoretical result that tell us what the sampling distribution should look like (for certain sample statistics)

- The Central Limit Theorem provides the foundation we need to conduct inference on parameters such as the mean, proportion, and regression coefficients (eventually)

---

### Central Limit Theorem 

If certain conditions are met, the sampling distribution of the sample statistic will be 
...
- Sample Statistic: 
```{r echo = F}
treatment <- sesame_street %>% filter(treatment == 1)
control <- sesame_street %>% filter(treatment == 2)
...
```{r echo = F}
obs_t <- sesame_street %>% 
  t_stat(formula = change ~ treatment, order = c(1, 2))
obs_t
```
