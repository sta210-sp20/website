...
- In the last class, we conducted a hypothesis test and concluded that children who watched *Sesame Street* regularly had a significantly greater improvement in reading scores, on average, than children who didn't want the show regularly. 

.instruction[
Today we will estimate how large the difference in average improvement really is. 
]
...
- <font class="vocab">**Why**</font>: Because the statistic is a random variable, its value is subject to chance error, i.e. random variability
  + We want to take that variability into account by reporting a range of plausible values rather than the single statistic  

---

### Recall: Central Limit Theorem 

- Using the <font class = "vocab">Central Limit Theorem (CLT)</font> we know the form of the the sampling distribution for certain statistics such as the mean, proportion, difference in means, etc.
    - It does not apply to all statistics. For example, it does not apply to order statistics like the median.
...
- In the *Sesame Street* scenario, the parameter of interest is the difference in means, $\mu_{1} - \mu_{2}$

- The statistic is the difference in sample means $\bar{x}_1 - \bar{x}_2$

- Assuming the conditions for the CLT are met ($n$ large), the sampling distribution for $\bar{x}_1 - \bar{x}_2$ is 
...
- Let's center the inequality around the parameter $\mu_1 - \mu_2$
...
<font class="vocab">*SE:*</font> standard error of the statistic
<br>

$$\text{Lower Bound} = \text{ Estimate} - \text{ (critical value) } \times SE$$
$$\text{Upper Bound} = \text{ Estimate} + \text{ (critical value) } \times SE$$
<br> 

Let's talk about the SE and the critical value
---
...
- We will use the sample standard deviations $s_1$ and $s_2$ instead

- The <font class="vocab">**standard error of $\mathbf{\bar{x}_1 = \bar{x}_2}$**</font> is
...
- The critical value is determined based on the parameter being estimated and the confidence interval. 

- In our context, we will use the $t$ distribution to calculate the critical value to adjust for the extra uncertainty that arises from using $s_1$ and $s_2$ instead of $\sigma_1$ and $\sigma_2$
...
<br>
<br>
In practice, we will use RStudio to determine the degrees of freedom.
...
- Intervals contain the parameter of interest 95% of the time

- It is <u>**not**</u> interpreted as the probability the parameter of interest is in a given interval
...
.question[
1. Interpret the interval in the context of the question. 
...
- Confidence intervals can be used instead of hypothesis tests to assess a claim
...
- Let's also suppose that the hypotheses are $H_0: \mu = 1$ vs. $H_a: \mu \neq 1$
    - If the 95% confidence interval contains the hypothesized value, 1, then this two-sided hypothesis test will result in a p-value that is greater than 0.05
    - If the 95% confidence interval does not contain the hypothesized value, 1, then this two-sided hypothesis test will result in a p-value that is less than 0.05
...
1. The statement, "the p-value is .003", is equivalent to the statement, "there is a 0.3% probability that the null hypothesis is true".

2. A p-value of .04 is more evidence against the null hypothesis than a p-value of .08.

3. If you increase sample size, the width of confidence intervals will increase.

4. For a given standard error, higher confidence levels (e.g. 99% vs. 95%) produce wider confidence intervals.
