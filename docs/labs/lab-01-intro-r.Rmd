...
What is the mean number of bike rentals? The middle 50% of bike rentals are between what two values? 
...
*This is a good place to pause and commit changes with the commit message "Added summary statistics  (Ex 1 and 2)", and push.*
...
What is the difference in the two plots? In other words, what is the difference in the way color is implemented in the two plots?

6. Describe the distribution of `count`. Your description, include comments about the shape, center, spread, and any potential outliers. You should use the histogram and the summary statistics from Exercise 2 in your description. 

*This is a another good place to pause and commit changes with the commit message "Added univariate data visualization (Ex 3 - 6)", and push.*

7. Now that we've examined the variables individually, we want to look at the relationship between the variables. Complete the code below to make a scatterplot of the number of bike rentals versus the temperature.

```{r eval=FALSE}
ggplot(data=bikeshare, mapping=aes(x=temp,fill=count)) +
  ___________________
```


```{marginfigure}
[https://ggplot2.tidyverse.org/](https://ggplot2.tidyverse.org/) is a great resource as you learn `ggplot()`. Click **Reference** in the top right corner to see a list of the various plot types available in the ggplot2 package.skeleton
```

Describe the relationship between the temperature and the number of bike rentals. 


8. The temperature and number of bike rentals varies greatly depending on the season. Therefore, we would like to create a separate scatterplot of `count` versus `temp` for each season. To do so, we will use the `facet_wrap()` function, faceting by `season`. Recall from Exercise 2 that `season` is currently stored as an integer. We will change it to a factor variable type, and use it in the `facet_wrap()` function.
...
```{r eval=FALSE}
ggplot(data=bikeshare, mapping=aes(x=temp,y=count)) + 
  geom_point() +
  facet_wrap(~season)
```

For which season is the linear relationship between the temperature and the number of bike rentals the strongest? Briefly explain your choice. 

*This is a another good place to pause and commit changes with the commit message "Added bivariate data visualization (Ex 7 - 8)", and push.*

## Simple Linear Regression 

We want to fit a least-squares regression using the temperature to explain variation in the number of bike rentals in the **winter** season. We can use the `filter()` function to create a subset from the data that only includes days during the winter. The `<-` assigns the name `winter_data` to our subset. 
...
9. Fit a simple linear regression model using the `lm()` function; assign it the name `winter_model`. Replace *X*,  *Y*, and *my.data* in the code below with the appropriate values. 

```{r eval=FALSE}
winter_model <- lm(Y ~ X, data=my.data)
```

Use the `tidy()` function to out the model in a "tidy" way. 
