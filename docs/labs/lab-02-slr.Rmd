...
Use the histogram and summary statistics to describe the distrbution of `cost`. 
...
5. Later in the semester, we will techniques to deal with missing values in the data. For now, however, we will only include complete observatons in the remainder of the analysis. We can use the `filter` function to select only the rows that values for both `cost` and `admrate`. 
...
7. The data contains information about the type of college, and we would like to see the relationship while also incorporprating information about the type of college. One way to do this is to use a different color for each type of college. Fill in the code below the scatterplot from Question 3 with the marker colors based on the variable `type`. Briefy describe how the relationship between `admrate` and `cost` compare across values of `type`. 
