...
8. Consider the variable `duration_ms`. 
    ...
    - Suppose instead of `duration_ms`, we use the variable `duration_sec`, the duration of a song in seconds. What would be the effect of `duration_s` on the odds of the user liking a song? Include the updated coefficient and corresponding 95% confidence interval for `duration_s`. *Assume all other variables in the model are unchanged.*
...
16. Now let's put your model and decision threshold to the test! Use your model to calculate the predicted probability (and corresponding interval) that the user will like the following two songs:
