...
In today's lab, we will focus on using song characteristics to determine whether or not a user will like a song. We will use logistic regression to build a model that predicts the probability a user likes a song using the relevant characteristics of the song. 
...
1. Use the `glimpse` function to see the size of the data set and the variable names. The response variable for this analysis is `like`, such that 1 indicates that the user likes the song and 0 otherwise. The remaining variables are potential predictor variables.
```{marginfigure}
Part of the code to make `like` a factor. 

```mutate(like = factor(like))```
```

    - Which variables are categorical? 
    - Make the response variable and each of the categorical predictors a `factor` variable type in the dataset. 
...
*In practice, you should do exploratory data analysis for all potential explanatory variables. For the sake of time on today's lab, we did an abbreviated exploratory data analysis to make the assignment more manageable.*
...
For the remainder of this lab, we will use the model chosen by model selection. In practice; however, we would not just stop with the results from the automated model selection procedure and would examine the model further to see if it can be simplified.
...
8. Consider the variable `duration_ms`. 
    ...
    - Suppose instead of `duration_ms`, we use the variable `duration_sec`, the duration of a song in seconds. What would be the coefficient and corresponding 95% confidence interval for `duration_s`? *Assume all other variables in the model are unchanged.*
...
10. Create a binned plot of the residuals versus the predicted probabilities. 
...
*We should examine plots of residuals versus <u>every</u> predictor variable to make a complete assessment of the model fit. For the sake of time on the lab, we will use these three plots to help make the assessment.*
...
Also include the appropriate interval. The data for the songs can be found in *test_songs.csv*. 

17. Using your decision threshold from Question 16, would you recommend "Sign of the Times" to this user? Would your recommend "Hotline Bling" to this user? 

18. The user likes "Hotline Bling" but doesn't like "Sign of the Times". How good were your recommendations based on these two songs? If they were good recommendations, explain how the model and threshold helped you distinguish between songs the user would like and those he wouldn't. If they were not good recommendations, explain the limitations in your model and/or threshold.
